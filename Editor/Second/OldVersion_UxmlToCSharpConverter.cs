...
namespace TCS.UiToolkitUtils.Editor {
    public class OldVersion_UxmlToCSharpConverter : EditorWindow {
        ...
        public static void ShowWindow() {
            var window = GetWindow<OldVersion_UxmlToCSharpConverter>( "UXML to C# Converter" );
        ...
        public void CreateGUI() {
            ...
            var togglesContainer = new VisualElement { style = { flexDirection = FlexDirection.Row, marginTop = 5 } };
            ...
            m_extractUssToggle = new Toggle( "Extract Inline Styles to USS file" ) { value = true, style = { marginTop = 8 } };
            ...
            m_setTextFieldsToggle = new Toggle( "Set Text Fields" ) { value = true, style = { marginTop = 8 } };
            ...
            m_allowDisplayNoneToggle = new Toggle( "Allow (display: none)" ) { value = false, style = { marginTop = 8 } };
            ...
            // configContainer.Add( m_extractUssToggle );
            // configContainer.Add( m_setTextFieldsToggle );
        ...
        void OnDestroy() {
            if ( m_namespaceField != null && m_namespaceChangeCallback != null ) {
                m_namespaceField.UnregisterCallback( m_namespaceChangeCallback );
            }

            if ( m_outputClassNameField != null && m_classNameChangeCallback != null ) {
                m_outputClassNameField.UnregisterCallback( m_classNameChangeCallback );
            }

            if ( m_extractUssToggle != null && m_extractUssToggleCallback != null ) {
                m_extractUssToggle.UnregisterValueChangedCallback( m_extractUssToggleCallback );
            }

            if ( m_setTextFieldsToggle != null && m_setTextFieldsCallback != null ) {
                m_setTextFieldsToggle.UnregisterValueChangedCallback( m_setTextFieldsCallback );
            }

            if ( m_allowDisplayNoneToggle != null && m_allowDisplayNoneCallback != null ) {
                m_allowDisplayNoneToggle.UnregisterValueChangedCallback( m_allowDisplayNoneCallback );
            }
        }
        ...
        void GenerateAndDisplayPreview() {
            ...
            try {
                ...
                if ( !instantiableElements.Any() ) {
                    ShowStatus( "No valid VisualElements found in UXML.", HelpBoxMessageType.Error );
                    return;
                }
        ...
        static void SetTextFields(IEnumerable<ElementInfo> elementInfos, StringBuilder sb, string indent) {
            ...
            // foreach (var info in elementInfos) {
            //     var textAttribute = info.XmlElement.Attribute("text");
            //     if (textAttribute != null && !string.IsNullOrWhiteSpace(textAttribute.Value) &&
            //         info.ElementType is "Label" or "Button" or "Toggle" or "Foldout" or "TextField" or "TextElement") {
            //         sb.AppendLine($"{indent}        {info.FieldName}.text = \"{textAttribute.Value.Replace("\"", "\\\"")}\";");
            //     }
            // }

            /*foreach (var info in elementInfos) {
                string attributeValue = info.XmlElement.Attribute("text")?.Value ?? info.XmlElement.Attribute("label")?.Value;
                if (!string.IsNullOrWhiteSpace(attributeValue)) {
                    string escapedValue = attributeValue.Replace("\\", "\\\\").Replace("\"", "\\\"").Replace("\n", "\\n").Replace("\r", "\\r");
                    switch (info.ElementType) {
                        case "Label" or "Button" or "Foldout" or "TextElement" or "Toggle" or "RadioButton":
                        case "TextField" when info.XmlElement.Attribute("text") != null:
                            sb.AppendLine($"{indent}        {info.FieldName}.text = \"{escapedValue}\";");
                            break;
                        case "TextField"
                            or "Slider" or "SliderInt" or "MinMaxSlider" or "DropdownField" or "EnumField"
                            or "RadioButtonGroup" or "IntegerField" or "FloatField" or "LongField" or "DoubleField"
                            or "Hash128Field" or "Vector2Field" or "Vector3Field" or "Vector4Field" or "RectField"
                            or "BoundsField" or "UnsignedIntegerField" or "UnsignedLongField" or "Vector2IntField"
                            or "Vector3IntField" or "RectIntField" or "BoundsIntField":
                            sb.AppendLine($"{indent}        {info.FieldName}.label = \"{escapedValue}\";");
                            break;
                    }
                }

                if (info.ElementType == "ProgressBar") {
                    string titleValue = info.XmlElement.Attribute("title")?.Value;
                    if (!string.IsNullOrWhiteSpace(titleValue)) {
                        string escapedValue = titleValue.Replace("\\", "\\\\").Replace("\"", "\\\"").Replace("\n", "\\n").Replace("\r", "\\r");
                        sb.AppendLine($"{indent}        {info.FieldName}.title = \"{escapedValue}\";");
                    }
                }
            }#1#
            #endregion
        ...
        #region Old GenerateFieldName Method
        ...
        string ToKebabCase(string str) {
            ...
            // This regex is equivalent to the one in the user's source generator.
            // It converts PascalCase to kebab-cases.
